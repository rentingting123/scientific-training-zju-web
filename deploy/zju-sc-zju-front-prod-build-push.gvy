pipeline { 
    ...
    environment {
        ...
        //构建机上的工作目录
        REMOTE_DIRECTORY = "/data/deploy/schools/zju-06-4133010335/front/zju-sc_prod"

        //构建机上要执行的命令,如有多个,用分号";"隔开
        EXEC_COMMAND = "zju-sc_prod.sh ${BRANCH_TAG}"

        //打出来的镜像名称
        IMG_NAME = "/zju/zju-sc:prod_${BRANCH_TAG}"
    }
    ...
    stages { 
        stage('Git Checkout') { 
            steps {
                git url:'git@codeup.aliyun.com:60813db3429bb594fdddcd87/front/scientific-training-web.git' ,branch:"${BRANCH_TAG}"
                sh "rm -fr ${REMOTE_DIRECTORY}/code/"
                sh "cp -a /var/jenkins_home/workspace/kyxl-schools/zju-sc-zju-front-prod-build-push ${REMOTE_DIRECTORY}/code/"
