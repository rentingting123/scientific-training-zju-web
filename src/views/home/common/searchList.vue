<template>
    <ul >
        <li v-for="item in datas" :key="item.id" @click="toDetail(item.id,item.noticeType)">
            <span></span>
            <p class="dian">{{item.title || item.name}}</p>
...
export default {
    ...
    methods:{
        toDetail(id,noticeType){
            let query = {}
            if(noticeType) query.type = noticeType
            if(noticeType == 11){
                ...
                this.$router.push({path:'/home/aboutlink/'+id,query:query})
