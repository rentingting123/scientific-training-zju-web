...
async function getDetail(id) {
  ...
  //成熟程度
  await StuApi.getDicts("cyxl_cscd").then(response => {
    response.data.map(item => {
  ...
  return StuApi.expertDetail({xmid:id,xmejlb:14})
    .then(data => {
      let _fd =Object.assign({},data.data,data.data.txmCyxl,data.data.txmBase) ;
      //学科类别
