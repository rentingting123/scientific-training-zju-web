...
<li class="noleft" v-if="data.xyjtyj">
...
<li>
    <span>学院结题意见: </span>
    <div>{{data.txmjt && data.txmjt.xyyj || '-'}}</div>
</li>
<li>
    <span>专家组成员: </span>
...
<li v-if="data.txmfj">
    ...
    <div class="right">
        <p v-for="item in data.txmfj" :key="item.id">{{item.filename}}</p>
    </div>
</li>
...
import reviewFactory from "./common/reviewFactory"
export default {
    components: {
        personDetail,
        CKReview
