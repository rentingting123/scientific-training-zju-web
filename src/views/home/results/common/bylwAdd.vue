<template>
    <div style="display:inline-block;margin-right:8px;">
        ...
        <el-dialog :visible.sync="show" :title="title + '毕业论文成果'" width="800" append-to-body>
            <el-form :model="form" ref="ruleForm" label-width="170px">
                ...
                <el-form-item label="成果简单描述" prop="bylwsj" :rules="[{ required: true, message: '请输入成果简单描述', trigger: 'blur' }]">
                    <el-input v-model="form.bylwsj" type="textarea" :disabled='!!id'></el-input>
                </el-form-item>
                <el-form-item label="学院、竞赛基地开具证明扫描上传" prop="fjurl" :rules="[{ required: true, message: '请上传学院、竞赛基地开具证明', trigger: 'blur' }]">
                    <template v-if="!id">
...
export default {
    ...
    methods:{
        xq(){
            this.form = {}
            network.bylwXq({id:this.$route.params.id}).then(res=>{
        ...
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (!valid)  return;
                network.addBylw({...this.form}).then(res=>{
