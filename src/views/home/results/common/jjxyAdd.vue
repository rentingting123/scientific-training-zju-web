<template>
    <div style="display:inline-block;margin-right:8px;">
        ...
        <el-dialog :visible.sync="show" :title="title+'经济效益'" width="800" append-to-body>
            <el-form :model="form" ref="ruleForm" label-width="170px">
                ...
                <el-form-item label="年产值(万元)" prop="ncz" :rules="[{ required: true, message: '请输入年产值', trigger: 'blur' }]">
                    <el-input v-model="form.ncz" :disabled='!!id'></el-input>
                </el-form-item>
                <el-form-item label="年利税(万元)" prop="nls" :rules="[{ required: true, message: '请输入年利税', trigger: 'blur' }]">
                    <el-input v-model="form.nls" :disabled='!!id'></el-input>
                </el-form-item>
                <el-form-item label="合同、财务凭证扫描上传" prop="fjurl" :rules="[{ required: true, message: '请上传合同、财务凭证', trigger: 'blur' }]">
                    <template v-if="!id">
...
export default {
    ...
    methods:{
        xq(){
            this.form = {}
            network.jjxyCqXq({id: this.id, cglb: 7}).then(res=>{
        ...
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (!valid)  return;
                // cglb 7经济效益 8其他类知识产权
                network.addJjxyCq({id:this.form.xmid,cglb:7,...this.form}).then(res=>{
