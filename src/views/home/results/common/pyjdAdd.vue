<template>
    <div style="display:inline-block;margin-right:8px;">
        ...
        <el-dialog :visible.sync="show" :title="title+'评议鉴定成果'" width="800" append-to-body>
            <el-form :model="form" ref="ruleForm" label-width="160px">
                ...
                <el-form-item label="评议、鉴定成果名称" prop="pyjdcgmc" :rules="[{ required: true, message: '请输入评议、鉴定成果名称', trigger: 'blur' }]">
                    <el-input v-model="form.pyjdcgmc" maxlength="200" :disabled='!!id'></el-input>
                </el-form-item>
                <el-form-item label="评议、鉴定单位" prop="pyjddw" :rules="[{ required: true, message: '请输入评议、鉴定单位', trigger: 'blur' }]">
                    <el-input v-model="form.pyjddw" maxlength="200" :disabled='!!id'></el-input>
                </el-form-item>
                <el-form-item label="评议、鉴定日期" prop="pyjdrq" :rules="[{ required: true, message: '请选择评议、鉴定日期', trigger: 'blur' }]">
                    <el-date-picker 
                        v-model="form.pyjdrq" 
                        align="right" 
                        type="date" 
                ...
                <el-form-item label="鉴定书扫描件上传" prop="fjurl" :rules="[{ required: true, message: '请上传鉴定书扫描件', trigger: 'blur' }]">
                    <template v-if="!id">
...
export default {
    ...
    methods:{
        xq(){
            this.form = {}
            network.pyjdXq({id: this.id}).then(res=>{
        ...
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (!valid)  return;
                network.addPyjd({...this.form}).then(res=>{
