<template>
    <div class="zscq">
        <el-table border :data="datas" border style="width: 100%">
            ...
            <el-table-column prop="qtl" label="其它类知识产权" min-width="100"></el-table-column>
...
export default {
    ...
    methods:{
        getLists(){
            network.getzscqCqist({id:this.$route.params.id}).then(res=>{
        ...
        del(row){
            this.$confirm('删除后无法恢复，请确认是否删除','提示',{
                ...
                network.delJjxyCq({id: row.id, cglb: 8}).then(res=>{
