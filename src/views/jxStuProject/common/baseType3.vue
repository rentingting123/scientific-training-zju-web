<template>
    <jxDialog  v-on="$listeners" v-bind="$attrs">
             <el-form
              ...
              <el-form-item>
                   <router-link to="/reading">
                          <span style="color:red;font-size:18px;">申报国创项目须知 </span>(点击阅读)
                    </router-link>
              </el-form-item>
        ...
        <el-row v-if="formData.xiangmuly==439">
            <el-col :span="12">
        ...
        <el-form-item label="注册公司时间（预计）:" prop="zcgssj">
            <el-date-picker
              v-model="formData.zcgssj"
              type="date"
        ...
        <el-form-item label="注册公司地点（预计）:" prop="zcgsdd">
             <el-input
                  v-model="formData.zcgsdd"
                  placeholder="请输入项目名称"
                  clearable
    ...
    font-size: 29px;"
        ...
        <el-form-item label="其他有关情况" prop="qtqk">
          <CKEditor
            v-model="formData.qtqk"
            type="textarea"
            placeholder="请填写，不超过200字项目主要内容简介："
            :maxlength="200"
            :autosize="{ minRows: 4, maxRows: 4 }"
        ...
        <el-form-item label="创业计划书:" prop="fileList">
                   <upload  
                     :uploaded="uploadFile"
                     accept=".doc,.docx">
                      <el-tag
                        :key="tag.key"
                        v-for="(tag,index) in formData.fileList"
                        closable
              ...
              <el-button v-if="formData.fileList.length <1" size="small" type="primary" icon="el-icon-upload">点击上传</el-button>
              <span v-if="formData.fileList.length <1" style="padding-left:20px">请上传以 .doc 或 .docx结尾的world文件</span>
            </upload>
        </el-form-item>
...
export default {
  components: { CKEditor ,jxDialog,upload},
  ...
  created() {
    ...
    //查询最多老师 学生
    StuApi.checkMaxMeber('gc_cysj_ds').then(d=>{this.maxTeacer = d.data})
    StuApi.checkMaxMeber('gc_cysj_xs').then(d=>{this.maxStudent = d.data})

    this.getDicts("cysj_project_from").then(response => {
      this.xiangmulyOptions = response.data;
    });
    //xiangmuxianzOptions
    this.getDicts("cysj_status").then(response => {
      this.xiangmuxianzOptions = response.data;
    });
    //cysj_cscd
     this.getDicts("cysj_cscd").then(response => {
  ...
  methods: {
    ...
    getDetailById(id){
        if(!id)return
        StuApi.expertDetail({xmid:id,xmejlb:13}).then(data=>{
            let _formData =Object.assign({},data.data.txmBase,data.data.txmCysj) 
            // console.log(_formData);
                _formData.cscdarr = _formData.cscd.split(',')
                try{
                  _formData.fileList = []
                  // _formData.fileList = JSON.parse(_formData.cyjhs)
                  // _formData.fileList.push( _formData.cyjhs)
    ...
    handelConfirm(isfb=1) {
      ...
      this.$refs["elForm"].validate(valid => {
       ...
       //计算属性经费
        ...
        this.formData.cyjhs = JSON.stringify(this.formData.fileList||[])
        //0保存1提交
        ...
        this.formData.xmejlb = 13//创业实践
        this.formData.type = 3//国创创业实践
        this.formData.txmCysj = ObjectUpdate(this.txmCysj,this.formData)
        if(this.id){
