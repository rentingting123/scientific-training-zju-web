      ...
      <el-table border v-loading="loading" :data="ProjectList">
        ...
        >
          <template slot-scope="{ row }">
            ...
            <div v-else-if="column.key == 'qrzt'">
              <div v-if="$store.getters.role !== 'student' && $store.getters.role !== 'blacklist'" >
              <el-button
                  v-if="row.xmzt == 12"
                  size="mini"
                  type="text"
                  icon="el-icon-edit"
                  @click="$router.push('/jxStuProject/results')"
              ...
              <div v-else>
                ...
                <el-button
                  size="mini"
                  type="text"
                  v-else-if="row.xmzt == 17 && row.fjlx == 3"
                  icon="el-icon-edit"
        ...
        >
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="text"
              v-if="scope.row.xmzt == 17 && scope.row.fjlx == 3"
              icon="el-icon-edit"
            ...
            <jxUpload
              accept=".pdf"
              :change="(val) => fileUpload(val, scope.row)"
            >
              <el-button size="small" type="text">上传结题报告</el-button>
...
export default {
  ...
  methods: {
    ...
    handlePDF({ id, xiangmumc, fzxsxh }) {
      ProjectApi.expertPdf({ id, type: "jt" }).then((data) => {
        downloadFile(data, fzxsxh + "_" + xiangmumc + "_结题表", "pdf");
      });
    },
